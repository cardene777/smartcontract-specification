...
コントラクトの初期設定（名前、シンボル、初期管理者等）を行うinitialize関数を提供。

### ロール管理

grantRole、revokeRoleによるロールの付与・剥奪。マルチシグ承認が必要な場合もあります。

### 設定変更

システムパラメータの変更機能。変更には適切な権限が必要です。

## 関数一覧

<details>
<summary><strong>定数 (14)</strong></summary>
...
| [`UPGRADE`](#upgrade) | UPGRADE_INTERFACE_VERSION 定数の値を取得します。 |
...
UPGRADE_INTERFACE_VERSION 定数の値を取得します。

**戻り値:**

| 名前 | 型 | 説明 |
|------|-----|------|
| `result0` | `string` | UPGRADE_INTERFACE_VERSIONの値を返します。 |
...
<details>
<summary><strong>読み取り関数 (15)</strong></summary>
...
| [`proxiableUUID`](#proxiableuuid) | proxiableUUID |
...
| 名前 | 型 | 説明 |
|------|-----|------|
| `result0` | `string` | proxiableUUIDの結果を返します。 |
...
| 関数名 | 説明 |
|--------|------|
| [`allowAccount`](#allowaccount) | allowAccount |
...
| [`disallowAccount`](#disallowaccount) | disallowAccount |
...
| [`unpause`](#unpause) | コントラクトの一時停止を解除します。 |
| [`unpauseBank`](#unpausebank) | 指定されたBankの一時停止を解除します。 |
| [`upgradeToAndCall`](#upgradetoandcall) | upgradeToAndCall |
...
```solidity
contract.disallowAccount(account);
```
...
| 名前 | 型 | 必須 | 説明 |
|------|-----|------|------|
| `newImplementation` | `any` | ✓ | 新しい実装コントラクトのアドレスを指定します。 |
...
```solidity
contract.upgradeToAndCall(newImplementation, data);
```
...
<details>
<summary><strong>イベント (29)</strong></summary>
...
コントラクトがアップグレードされた時に発行されるイベントです。新しい実装アドレスが記録されます。
...
<details>
<summary><strong>エラー (57)</strong></summary>
...
| `errors/ERC1967InvalidImplementation` | 無効な実装アドレスが指定された時に返されるエラーです。 |
| `errors/ERC1967NonPayable` | ETHの受け取りが許可されていない時に返されるエラーです。 |
...
| `errors/UUPSUnauthorizedCallContext` | UUPSアップグレードが不正なコンテキストから呼び出された時に返されるエラーです。 |
| `errors/UUPSUnsupportedProxiableUUID` | プロキシが互換性のない実装をアップグレードしようとした時に返されるエラーです。 |
