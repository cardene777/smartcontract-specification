...
> **[📋 API仕様書を見る](/api/StablecoinBank)**

## 概要

複数のBank（発行元）による分散発行をサポートする機能です。
...
新しいBankの登録、発行上限（cap）の設定、Bank情報の更新を管理。

### 発行残高追跡

各Bankの発行済み残高（outstanding）を追跡。発行上限との比較で発行可能量を判定。

### Bank別ロール

各BankにISSUER、BURNER、PAUSER等のロールを個別に割り当て可能。Bank単位での権限分離を実現。
...
<details>
<summary><strong>読み取り関数 (21)</strong></summary>

| 関数名 | 説明 |
|--------|------|
| [`bankBalanceOf`](#bankbalanceof) | bankBalanceOf |
| [`bankCap`](#bankcap) | bankCap |
| [`bankInfo`](#bankinfo) | bankInfo |
| [`bankTotalAmount`](#banktotalamount) | bankTotalAmount |
...
| [`getBankId`](#getbankid) | bank id を取得します。 |
...
| 名前 | 型 | 説明 |
|------|-----|------|
| `result0` | `string` | bankCapの結果を返します。 |
...
<details>
<summary><strong>書き込み関数 (28)</strong></summary>

| 関数名 | 説明 |
|--------|------|
| [`addBank`](#addbank) | addBank |
...
| [`grantBankRoleToAccount`](#grantbankroletoaccount) | grantBankRoleToAccount |
...
| [`pauseOwnBank`](#pauseownbank) | pauseOwnBank |
...
| [`revokeBankRoleFromAccount`](#revokebankrolefromaccount) | revokeBankRoleFromAccount |
| [`revokeRole`](#revokerole) | 指定されたアカウントからロールを取り消します。 |
| [`setBankCap`](#setbankcap) | setBankCap |
...
```solidity
contract.grantBankRoleToAccount(account, bankId);
```
...
|------|-----|------|------|
| `bankId` | `any` | ✓ | 銀行のIDを指定します。 |
| `targetBankId` | `any` | ✓ | targetBankIdを指定します。 |
...
```solidity
contract.removeBank(bankId, targetBankId);
```
...
```solidity
contract.revokeBankRoleFromAccount(account, bankId);
```
...
```solidity
contract.setBankCap(bankId, cap);
```
...
<details>
<summary><strong>イベント (35)</strong></summary>
...
銀行のキャップが設定または変更された時に発行されるイベントです。銀行IDと新しいキャップが記録されます。

---

### events/BankDeactivated

銀行が非アクティブ化された時に発行されるイベントです。対象の銀行IDが記録されます。
...
### events/BankRegistered

新しい銀行が登録された時に発行されるイベントです。銀行ID、名前、キャップなどが記録されます。

---

### events/BankRemoved

銀行が削除された時に発行されるイベントです。削除された銀行のIDが記録されます。
...
### events/BankUnpaused_1

銀行の一時停止が解除された時に発行されるイベントです。銀行IDと解除を実行したアカウントが記録されます。

---

### events/BucketReassigned

バケットが再割り当てされた時に発行されるイベントです。元の所有者、新しい所有者、金額が記録されます。
