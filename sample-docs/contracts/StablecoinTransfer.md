...
> **[📋 API仕様書を見る](/api/StablecoinTransfer)**

## 概要

トークン転送に関する拡張機能を提供します。コンプライアンス要件への対応を含みます。
...
FORCE_TRANSFER_ROLEを持つアカウントが任意のアドレス間でトークンを強制的に移動。法的要件への対応に使用します。

### アローリスト

ALLOWLIST_ROLEでアカウントをホワイトリストに追加。許可されたアカウントのみが転送可能な制限モードを実現。

### アカウント凍結

FREEZER_ROLEで特定アカウントの転送を禁止。不正行為への対応に使用します。
...
<details>
<summary><strong>書き込み関数 (23)</strong></summary>
...
| [`forcedTransfer`](#forcedtransfer) | forcedTransfer |
| [`forcedTransferWithBank`](#forcedtransferwithbank) | forcedTransferWithBank |
...
| [`transferWithBank`](#transferwithbank) | transferWithBank |
...
```solidity
contract.forcedTransfer(from, to, amount);
```

---

### forcedTransferWithBank
...
```solidity
contract.forcedTransferWithBank(from, to, amount, bankId);
```
...
| 名前 | 型 | 説明 |
|------|-----|------|
| `success` | `string` | successを返します。 |

**使用例:**

```solidity
contract.transferWithBank(to, amount, bankId);
```
...
<details>
<summary><strong>イベント (31)</strong></summary>
...
銀行間でトークンが転送された時に発行されるイベントです。転送元銀行、転送先銀行、金額が記録されます。
...
### events/ForcedTransfer

強制転送が実行された時に発行されるイベントです。送金元、送金先、金額が記録されます。
...
### events/TransferWithinBank

同一銀行内でトークンが転送された時に発行されるイベントです。銀行ID、送金元、送金先、金額が記録されます。
