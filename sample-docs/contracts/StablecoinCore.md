...
> **[📋 API仕様書を見る](/api/StablecoinCore)**

## 概要

ステーブルコインの中核となるコントラクトです。全ての機能モジュールを統合し、ERC20準拠のトークンとして動作します。
...
標準的なERC20インターフェース（transfer, approve, transferFrom等）を実装。Soladyライブラリによる効率的な実装を採用しています。

### ロールベースアクセス制御

ISSUER_ROLE、BURNER_ROLE、PAUSER_ROLE等の複数ロールによる細かな権限管理を実現。各ロールに対応した操作のみが許可されます。

### Bank機能

複数のBank（発行元）による分散発行をサポート。各Bankは独自の発行上限（cap）と発行残高を持ち、独立して運用できます。

### デュアルキーマルチシグ

重要な操作（ロール変更、キーローテーション等）にはプライマリキーとセカンダリキーの両方の承認が必要。セキュリティを強化します。
...
| [`implementation`](#implementation) | 現在の実装コントラクトのアドレスを取得します。 |
...
現在の実装コントラクトのアドレスを取得します。

**戻り値:**

| 名前 | 型 | 説明 |
|------|-----|------|
| `impl` | `string` | implを返します。 |
...
<details>
<summary><strong>書き込み関数 (21)</strong></summary>
...
| 名前 | 型 | 必須 | 説明 |
|------|-----|------|------|
| `name_` | `any` | ✓ | トークン名を指定します。 |
| `symbol_` | `any` | ✓ | トークンシンボルを指定します。 |
| `trustBankPrimary` | `any` | ✓ | trustBankPrimaryを指定します。 |
| `trustBankBackup` | `any` | ✓ | trustBankBackupを指定します。 |
| `developerPrimary` | `any` | ✓ | developerPrimaryを指定します。 |
| `developerBackup` | `any` | ✓ | developerBackupを指定します。 |
...
```solidity
contract.initialize(name_, symbol_, trustBankPrimary, trustBankBackup, developerPrimary, developerBackup);
```
