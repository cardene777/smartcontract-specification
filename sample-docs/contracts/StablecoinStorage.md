...
> **[📋 API仕様書を見る](/api/StablecoinStorage)**

## 概要

ステーブルコインの全状態変数を定義するストレージコントラクトです。
...
アップグレード時のストレージ衝突を防ぐため、全ての状態変数を一元管理。新しい変数は既存レイアウトの末尾に追加されます。

### 定数定義

ロール識別子（ISSUER_ROLE等）、提案の有効期限、必要承認数などの定数を定義。システム全体で一貫した値を使用します。
